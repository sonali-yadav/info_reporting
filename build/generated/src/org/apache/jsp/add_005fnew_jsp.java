...
public final class add_005fnew_jsp extends org.apache.jasper.runtime.HttpJspBase
  ...
  try {
      ...
      out.write("                    <div><label for=\"description\">Description:</label></div>\n");
      out.write("                    <div><input type=\"text\" name=\"description\" id=\"description\" required=\"required\"></div>\n");
      out.write("                    <div><label for=\"doc_link\">Link to Document:</label>\n");
      out.write("                    <div><input type=\"text\" name=\"doc_link\" id=\"doc_link\"></div>\n");
      out.write("                    <div><label for=\"faq_link\">Link to FAQ:</label>\n");
      out.write("                    <div><input type=\"text\" name=\"faq_link\" id=\"faq_link\"></div>\n");
      out.write("                    <div><label>Visible to? (Check all that apply)</label></div>\n");
      out.write("                    <div>\n");
      out.write("                        <div>\n");
      ...
      out.write("        ");
                ...
                if(ld.equals("")) { ld="NULL"; }
                if(lf.equals("")) { lf="NULL";}
                String[] access=request.getParameterValues("audience");
                out.println("desc= "+desc);out.println("ld= "+ld);out.println("lf= "+lf);
                for(String i: access){
                if(i.equals("s")) { s=1;}
                else if(i.equals("f")) { f=1;}
                else if(i.equals("e")) { e=1;}
                else if(i.equals("a")) { a=1;}
                    }
                String access_code = ""+s+f+e+a;
                out.println("access_code= "+access_code);
                //PreparedStatement ps=con.prepareStatement("select title_id from titles_tb where title_name='"+request.getParameter("title")+"'");
                //ResultSet r=ps.executeQuery();
                //String title_id="";
                //while(r.next()) { 
                  //  title_id=r.getString(1);
                    ////out.println("title_id= "+title_id);
                //}
                out.println("head= "+head);
                if(!(desc.equals(null)&& head.equals("")))
                {
                sql="insert into subtitles_tb(sub_name,title_id,last_modified,access_code,sub_doc,faq_doc) values('"+desc+"',"+title_id+",CURDATE(),"+access_code+","+ld+","+lf+")";
out.println(sql);                
